...
class CCNNModel(object):
    ...
    def __init__(self, shape):
        ...
        # self.alpha_f = 0.2 # exponential decay factor
        # self.alpha_theta = 0.2
        # self.alpha_l = 0.5
        # self.f = np.exp(-self.alpha_f) # exponential decay factor
        # self.g = np.exp(-self.alpha_theta)
        # self.h = np.exp(-self.alpha_l)

        # self.v_f = 0.1
        # self.v_l = 0.2
        # self.v_theta = 6

        self.alpha_f = np.log10(1 / delta)
        self.alpha_l = np.log10(1 / delta)
        self.v_l = 0.2 #1 in paper but can adjust
        self.v_f = 0.1 #fully didn't say in the paper
        self.beta = ((1.5 * S_max / S_prime - 1)) / (6 * self.v_l)
        self.v_theta = np.exp(-self.alpha_f) + 1 + (6 * self.beta * self.v_l)
        self.alpha_theta = np.log((self.v_theta / S_prime) / (((1 - np.exp(-3 * self.alpha_f)) / (1 - np.exp(-self.alpha_f))) + 6 * self.beta * self.v_l * np.exp(-self.alpha_f)))
    ...
    def segment_image(self, image, gamma, beta, v_theta, kernel_type, kernel_size=3, activation="classic"):
        ...
        while firednum < N:
            ...
            # self.Theta = self.Theta - ((np.exp(-self.alpha_theta) * self.Theta) + (self.v_theta * self.Y))
            flag = 1
            while flag == 1:
                Q = self.Y

                self.U = image * ((np.exp(-self.alpha_f) * self.U) + (self.F * (1 + self.beta * self.L)))
    ...
    def simulate(self, image, steps=5, step_size=1):
